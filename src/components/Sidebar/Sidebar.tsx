...
import type { SidebarIconSection, SidebarIcon } from "./SidebarTypes";
import "./Sidebar.css";

type SidebarProps = {
  sections: Array<SidebarIconSection>;
  open: boolean;
  setOpen: (newValue: boolean) => void;
};

export default function Sidebar({ sections, open, setOpen }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`sidebar-header ${open ? "open" : ""}`}>
        <img
          src="/coldview-logo-positivo.jpg"
          alt="Company Logo"
          className={`sidebar-logo ${open ? "open" : ""}`}
        />
        <span className={`sidebar-header-text ${open ? "open" : ""}`}>
          Coldview
        </span>
      </div>
      ...
      <div className="sidebar-content">
        {sections.map((section, index) => (
          <div key={index} className="sidebar-section">
            {section.showTitle && (
              <span className={`sidebar-section-title ${open ? "open" : ""}`}>
                {section.sectionTitle}
              </span>
            )}
            <div className={`sidebar-icon-list ${open ? "open" : ""}`}>
              {section.icons.map((icon: SidebarIcon, idx) => (  
                <div key={idx}>
                  <Link to={icon.route} className={`sidebar-icon-link ${open ? "open" : ""}`}>
                    <i className={`pi ${icon.icon}`}></i>
                    <span className={`sidebar-icon-name ${open ? "open" : ""}`}>
                      {icon.name}
                    </span>
                  </Link>
                </div>
                ...
                <div className="sidebar-footer-contact"> 
                    ...
                    <div className="sidebar-footer-contact-option-list">
                        <div className="sidebar-footer-contact-option">
                            ...
                            <span>1122529233</span>
                        </div>
                    </div>
                </div>
                <div className="sidebar-footer-contact"> 
                    <span className="sidebar-footer-contact-name">Central asistencia</span>
                    <div className="sidebar-footer-contact-option-list">
                        <div className="sidebar-footer-contact-option">
                            <i className="pi pi-phone" style={{fontSize: '0.8rem'}}/>
                            <span>1122529233</span>
